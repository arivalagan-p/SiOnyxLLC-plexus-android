...
public class ZoomableTextureView extends TextureView {
    ...
    private final int doubleTapTimeThreshold = 300;
...
//                    break;
    ...
    public void reset() {
        if (detectorOw != null) {
            ...
            saveScale = 1.0f;
            float mScaleFactor = detectorOw.getScaleFactor();
            float origScale = saveScale;
            saveScale *= mScaleFactor;
            mScaleFactor = 1.0f;
            minScale = 1f;
            ...
            right = getWidth() * saveScale - getWidth();
            bottom = getHeight() * saveScale - getHeight();
            matrix.postScale(mScaleFactor, mScaleFactor, detectorOw.getFocusX(), detectorOw.getFocusY());
            matrix.getValues(m);
            float x = m[Matrix.MTRANS_X];
            float y = m[Matrix.MTRANS_Y];
            if (0 < getWidth()) {
